...
export class AdminService {
  ...
  async deleteBusiness(id: string) {
    return this.prisma.$transaction(async (tx) => {
      const business = await tx.business.update({ where: { id }, data: { status: BusinessStatus.CANCELLED } });
      await tx.businessUser.updateMany({ where: { businessId: id }, data: { status: MembershipStatus.INACTIVE } });
      return business;
    });
  }
  async users() {
