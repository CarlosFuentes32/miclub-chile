...
La cuenta quedó conectada con GitHub y ofrece una prueba limitada a 30 días o US$5 de consumo, sin tarjeta. La API y PostgreSQL están publicados temporalmente en:

- API: https://miclub-api-production.up.railway.app/api
- Salud: https://miclub-api-production.up.railway.app/api/health

Antes de terminar el trial será necesario elegir un plan que mantenga los servicios activos y permita conservar el dominio personalizado. Las migraciones y el seed se ejecutan automáticamente durante el despliegue.

## Orden de los próximos pasos

1. Revisar el detalle del pedido de DonWeb y pagar solo cuando se autorice.
2. Probar login y el flujo completo con la URL temporal de Railway.
3. Comprar el dominio y agregarlo a Cloudflare Free.
4. Crear DNS para `miclubchile.cl`, `app`, `comercio`, `cajero`, `admin` y `api`.
5. Asociar cada dominio a su proyecto de Vercel y `api.miclubchile.cl` a Railway.
