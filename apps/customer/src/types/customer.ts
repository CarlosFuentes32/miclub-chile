...
export interface LoyaltyProgress { businessId: string; business: string; current: number; goal: number; reward: string; unit: string; active: boolean; }
...
export interface CustomerDashboard { programs: LoyaltyProgress[]; rewards: Reward[]; history: HistoryEntry[]; qrToken: string; shortCode: string; }
