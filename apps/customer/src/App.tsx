    ...
    return (
      <main className="grid min-h-screen place-items-center p-6 text-center">
        <div>
          ...
          <p className="mt-2 text-slate-500">Tu cuenta no puede acceder al Panel Cliente.</p>
          <a href={portalByRole[user.role]} className="mt-5 inline-block font-bold text-violet-700">
            Ir a tu portal
          </a>
        </div>
      </main>
  ...
  useEffect(() => {
    if (user) {
      customerService
        .getDashboard()
        .then(setDashboard)
        .catch((error) => setDataError(error instanceof Error ? error.message : 'Error de conexión'));
    } else {
      setDashboard(null);
    }
  }, [user]);
      ...
      <Route element={<Protected user={user} />}>
        <Route
          ...
          element={
            dashboard ? (
              <QRScreen
                profile={{ ...customerService.getProfile(user!), shortCode: dashboard.shortCode }}
                token={dashboard.qrToken}
              />
            ) : (
              <main className="grid min-h-screen place-items-center">Cargando QR…</main>
            )
          }
        />
          ...
          <Route
            ...
            element={
              dashboard ? (
                ...
                <main className="p-6 text-center">{dataError || 'Cargando beneficios…'}</main>
              )
            }
          />
